...
- Maintain a **public GitHub repository** with a meaningful history: **at least 8 commits**. Only commits that change test-script files (`.spec.js`, `.spec.ts`, or equivalent) count toward the 8-commit minimum; commits touching only the README, PDF, or other non-test documents do not count. _(Amended 2026-08-09: the instructor removed the "span over 4 days or more" constraint — the 8-commit minimum still applies.)_
