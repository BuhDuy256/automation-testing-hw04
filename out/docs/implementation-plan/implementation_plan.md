...
`hostname`. HW04 §7 additionally asks that the Agent Skill be demonstrated end to end on a complete
feature.

Two episodes are already written up and make the strongest narration material:
- `out/reports/FR-15-product-crud/automation/report.md` §11 — a **false pass**: a strict-equality
  assertion could not see a value the SUT returned as a string, so a test reported success on a
  defect that was present.
- `out/reports/FR-08-checkout/automation/report.md` §10.3 — four harness timeouts that, taken at face
...
machine — `pandoc`, `wkhtmltopdf`, `libreoffice`/`soffice` and `md-to-pdf` are all absent — and
nothing was installed. `out/README.md` §8 states that Markdown is authoritative and PDF conversion,
if Moodle requires it, must be done manually. The Playwright HTML reports are unaffected.

**Final figures.** 49 cases · 147 executions · 60 pass / 87 fail · **48 browser runs** (not 147) ·
**12 confirmed defects**, issues #1–#12. Ledger: **16 `.spec.ts` commits, 10 freezes**, against §12's
minimum of 8.
