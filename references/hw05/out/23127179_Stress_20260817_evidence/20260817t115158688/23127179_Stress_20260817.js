...
import { buildStressMarkdown } from './stress_stage_report.js';

const BASE_URL = __ENV.BASE_URL || 'http://localhost:3000';
const RUN_ID = __ENV.K6_RUN_ID || '';
const MARKDOWN_REPORT_PATH = __ENV.STRESS_MARKDOWN_REPORT_PATH || '';

if (!/^[a-z0-9]{8,20}$/.test(RUN_ID)) {
  throw new Error('K6_RUN_ID must match ^[a-z0-9]{8,20}$ and be unique for this invocation');
}
if (!MARKDOWN_REPORT_PATH.toLowerCase().endsWith('.md')) {
  throw new Error('STRESS_MARKDOWN_REPORT_PATH must identify the invocation-specific Markdown report');
}
...
const STRESS_PHASES = [
  { level: 'transition_1_to_4', targetVUs: 4, startMs: 0, endMs: 30_000, measurement: false },
  { level: 'baseline_4', targetVUs: 4, startMs: 30_000, endMs: 150_000, measurement: true },
  { level: 'transition_4_to_8', targetVUs: 8, startMs: 150_000, endMs: 180_000, measurement: false },
  { level: 'anchor_8', targetVUs: 8, startMs: 180_000, endMs: 300_000, measurement: true },
  { level: 'transition_8_to_12', targetVUs: 12, startMs: 300_000, endMs: 330_000, measurement: false },
  { level: 'level_12', targetVUs: 12, startMs: 330_000, endMs: 450_000, measurement: true },
  { level: 'transition_12_to_16', targetVUs: 16, startMs: 450_000, endMs: 480_000, measurement: false },
  { level: 'level_16', targetVUs: 16, startMs: 480_000, endMs: 600_000, measurement: true },
  { level: 'transition_16_to_20', targetVUs: 20, startMs: 600_000, endMs: 630_000, measurement: false },
  { level: 'level_20', targetVUs: 20, startMs: 630_000, endMs: 750_000, measurement: true },
  { level: 'transition_20_to_24', targetVUs: 24, startMs: 750_000, endMs: 780_000, measurement: false },
  { level: 'maximum_24', targetVUs: 24, startMs: 780_000, endMs: 900_000, measurement: true },
  { level: 'transition_24_to_4', targetVUs: 4, startMs: 900_000, endMs: 960_000, measurement: false },
  { level: 'recovery_4', targetVUs: 4, startMs: 960_000, endMs: 1_080_000, measurement: true },
  { level: 'transition_4_to_0', targetVUs: 0, startMs: 1_080_000, endMs: 1_140_000, measurement: false },
];

const MEASUREMENT_PHASES = STRESS_PHASES.filter((phase) => phase.measurement);
...
export const options = {
  scenarios: {
    stress: {
      ...
      gracefulStop: '30s',
      tags: { test_phase: 'official_stress' },
    },
  },
  thresholds: {
...
const crossStageIterations = new Counter('stress_cross_stage_iterations');
...
function requestParams(step, token, phase) {
  ...
  exec.vu.tags.stress_level = phase.level;
  return { headers, tags: { step, stress_level: phase.level } };
}

function recordResponse(response, step, phase) {
  const tags = { step, stress_level: phase.level };
  endpointLatency[step].add(response.timings.duration, tags);
  const metrics = stageMetrics[phase.level];
  if (!metrics) {
    return;
  }
...
function finishWorkflow(success, iterationStartedAt, iterationStartLevel) {
  const phase = currentStressPhase();
  ...
  if (metrics) {
    ...
    if (iterationStartLevel === phase.level) {
      metrics.iterationDuration.add(Date.now() - iterationStartedAt, tags);
    }
  }

  if (iterationStartLevel !== phase.level) {
    crossStageIterations.add(1, {
      start_level: iterationStartLevel,
      end_level: phase.level,
...
export default function () {
  ...
  const iterationStartLevel = currentStressPhase().level;
  exec.vu.tags.stress_level = iterationStartLevel;
  ...
  const registerPhase = currentStressPhase();
  ...
  if (!trackedCheck(registerResponse, {
    ...
    finishWorkflow(false, iterationStartedAt, iterationStartLevel);
    return;
  }

  randomPause(1, 2);

  const loginPhase = currentStressPhase();
  ...
  const profilePhase = currentStressPhase();
  ...
  const updatePhase = currentStressPhase();
  ...
  const categoriesPhase = currentStressPhase();
  ...
  const productsPhase = currentStressPhase();
  ...
  const detailPhase = currentStressPhase();
  ...
  const cartPhase = currentStressPhase();
  ...
  const checkoutPhase = currentStressPhase();
  ...
  finishWorkflow(true, iterationStartedAt, iterationStartLevel);
}

export function handleSummary(data) {
  return {
    [MARKDOWN_REPORT_PATH]: buildStressMarkdown(data, RUN_ID, MEASUREMENT_PHASES, STEPS),
    stdout: `STRESS_REPORT=${MARKDOWN_REPORT_PATH} K6_RUN_ID=${RUN_ID}\n`,
  };
}
