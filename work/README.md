...
npm run hw06:derive
node scripts/hw06/ai-audit.mjs capture --tool Codex --kind TEST_GENERATION --task "..." --prompt-file ... --output-file ...
node scripts/hw06/run-newman.mjs --collection ... --label ... --hostname localhost:3000
node scripts/hw06/capture-screenshot.mjs capture --id EVID-... --type ... --description "..." --url ...
node scripts/hw06/publish-bug.mjs preview --bug-id BUG-...
node scripts/hw06/capture-ci-run.mjs capture --run ... --purpose ... --artifact-name ... --screenshot-evidence-id ...
powershell -File scripts/hw06/build-submission.ps1 -Grade 090
```

The trigger, preconditions, procedure, validation, and fallback for each command are defined in
`docs/hw06-standard-actions.md`. Follow the action entry, not only the command example.
